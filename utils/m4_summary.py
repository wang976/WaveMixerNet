...
def group_values(values, groups, group_name):
    return np.array([v[~np.isnan(v)] for v in values[groups == group_name]], dtype = object) # murad: mod
...
class M4Summary:
    ...
    def evaluate(self):
        ...
        naive2_forecasts = np.array([v[~np.isnan(v)] for v in naive2_forecasts], dtype = object) # murad:L dtype object added
        ...
        for group_name in M4Meta.seasonal_patterns:
            ...
            insample = group_values(self.training_set.values, self.test_set.groups, group_name) # model input set
